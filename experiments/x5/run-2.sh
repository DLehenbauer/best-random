...
execute_practrand() {
    ...
    local output
    output=$( "$rng_exec" -p "$@" | stdbuf -oL "$practrand_exe" stdin64 -tlmin $pr_min -tlmax $pr_max 2>&1 ) || true

    # Collect FAIL lines (may include severity markers like ! or !!)
    local fail_lines
    fail_lines=$(echo "$output" | grep -iE '\bFAIL\b' || true)
    ...
    # Use Perl big number logic to avoid underflow for extremely small p-values (e.g. 1e-1785) and robustly
    # handle forms like 1-1e-10 by computing distance = min(p, 1-p) without relying on IEEE double or bc scale.
...
export rng_exec practrand_exe pass_file pr_min pr_max

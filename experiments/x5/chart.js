 ...
 *   4 .
 *
 * Color progression (green -> bright green -> yellow -> bright red -> red).
...
function severityOf(status) {
  ...
  if (s.includes('extremely')) return 4;               // EXTREMELY Worrying and very unusual
  if (s.includes('worrying')) return 3;                // Worrying and very unusual
  if (s.includes('very')) return 2;                    // very unusual
  if (s.includes('unusual')) return 1;                 // unusual
  return 0;                                            // ok
}
...
function cellSeverity(blocks, cellStart, cellEnd) {
  ...
  for (const b of blocks) {
    if (b.bytes >= cellStart && b.bytes < cellEnd) {
      if (b.sev > maxSev) maxSev = b.sev;
    }
  }
  if (maxSev >= 0) return maxSev;
  // fallback to last before cellStart
  let lastBefore = null;
  for (const b of blocks) {
    if (b.bytes < cellStart) lastBefore = b; else break;
  }
...
function buildBar(maxBytes, finalBytes, blocks, useColor) {
  ...
  // not to render '.' in the final bar position.
  ...
  while (i < finalPos) {
    ...
    let j = i + 1;
    while (j < finalPos && sevArr[j] === runSev) j++;
    const runLen = j - i;
...
function main() {
  ...
  if (!useColor) {
    console.log('');
    console.log('Colors suppressed (use TTY or omit --no-color).');
  } else {
    console.log('');
    console.log('Color scale: green -> bright green -> yellow -> bright red -> red');
  }
  console.log('');
  console.log('Cell severity = MAX severity among snapshots inside cell span; if none, last snapshot before span start.');
  console.log('Reversions lower later cells unless higher-severity snapshots also lie in their span.');
}

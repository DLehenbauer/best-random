...
uint32_t mix(uint32_t a, uint32_t b) {
    ...
    // a -= rot(9847 * b, 16); // len=0xfffe2f50 100.00%

    /* Top 25 primes < 1000: */
    // a -= rot(433 * b, 9);   // len=0xfff37585 99.98% (SC+: 0.028062/0.013300, 0.005054/0.019454, C-: 0.001674/0.002793, 0.000903/0.002045 HR: 1xU @ 512mb,32gb)
    // a -= rot(971 * b, 3);   // len=0xffdf29f9 99.95% (SC-: 0.002336/0.009153, 0.000599/0.037698, 64: Fail @ 256mb)
    // a -= rot(953 * b, 9);   // len=0xffdef236 99.95% (SC-: 0.002319/0.000684, 0.003212/0.050576, HR: 1xU @ 16mb)
    // a -= rot(449 * b, 26);  // len=0xffdc2b4c 99.95% (SC-: 0.026116/0.078190, 0.000947/0.024073, L: 2xU @ 16mb, 1xU @ 64mb)
    // a -= rot(499 * b, 6);   // len=0xffcea955 99.92% (SC+: 0.075314/0.083504, 0.022206/0.112144, C+: 0.002286/0.008522, 0.007206/0.005046, L: 1xU @ 512mb, 64: Fail @ 128gb)
    a -= rot(349 * b, 3);   // len=0xffbbba6e 99.90% (SC+: 0.024073/0.133245, 0.008962/0.086370, L: 1xU @ 8mb, 64: 1xS @ 64mb)
    // a -= rot(103 * b, 16);  // len=0xffa82671 99.87%
    // a -= rot(809 * b, 25);  // len=0xffa024a0 99.85%
    // a -= rot(439 * b, 15);  // len=0xff871d38 99.82%
    // a -= rot(457 * b, 25);  // len=0xff703c55 99.78%
    // a -= rot(283 * b, 16);  // len=0xff6f1b67 99.78%
    // a -= rot(709 * b, 16);  // len=0xff5d8140 99.75%
    // a -= rot(677 * b, 1);   // len=0xff519d88 99.73%
    // a -= rot(463 * b, 23);  // len=0xff46b695 99.72%
    // a -= rot(773 * b, 22);  // len=0xff33d161 99.69%
    // a -= rot(521 * b, 10);  // len=0xff2dba58 99.68%
    // a -= rot(811 * b, 25);  // len=0xff2c4df0 99.68%
    // a -= rot(857 * b, 19);  // len=0xff2a92b4 99.67%
    // a -= rot(373 * b, 1);   // len=0xff248c17 99.67%
    // a -= rot(151 * b, 2);   // len=0xff2088d0 99.66%
    // a -= rot(683 * b, 12);  // len=0xff0e9000 99.63%
    // a -= rot(59 * b, 31);   // len=0xff01ef9a 99.61%
    // a -= rot(811 * b, 3);   // len=0xff01bc42 99.61%
    // a -= rot(563 * b, 25);  // len=0xfef482f4 99.59%
    // a -= rot(887 * b, 26);  // len=0xfeef5964 99.58%
